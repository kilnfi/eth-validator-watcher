...
def test_get_validator_index_to_pubkey():
    beacon_url = "http://beacon:5052"
    validators_path = Path(assets.__file__).parent / "validators.json"
    ...
    with requests_mock.Mocker() as mock:
        mock.get(f"{beacon_url}/eth/v1/beacon/states/head/validators", json=validators)
        beacon = Beacon(beacon_url)

        assert beacon.get_active_validator_index_to_pubkey(input) == expected
